...
describe('collaborators queries', () => {
  ...
  describe('getCollaborators', () => {
    ...
    it('throws if Supabase returns an error', async () => {
      ...
      await expect(getCollaborators(VALID_UUID)).rejects.toThrow('fetch failed');
    });
  });
  ...
  describe('addCollaborator', () => {
    ...
    it('throws if insert returns an error', async () => {
      ...
      await expect(addCollaborator(VALID_UUID, VALID_UUID)).rejects.toThrow('insert failed');
    });
  });
  ...
  describe('removeCollaborator', () => {
    it('deletes the collaborator on success', async () => {
      ...
      await expect(removeCollaborator(VALID_UUID, VALID_UUID)).resolves.toBeUndefined();

      expect(supabase.from).toHaveBeenCalledWith('box_collaborators');
      expect(query.delete).toHaveBeenCalled();
      expect(query.eq).toHaveBeenCalledWith('box_id', VALID_UUID);
      expect(query.eq).toHaveBeenCalledWith('collaborator_profile_id', VALID_UUID);
    });
    ...
    it('throws if delete returns an error', async () => {
      ...
      await expect(removeCollaborator(VALID_UUID, VALID_UUID)).rejects.toThrow('delete failed');
    });
  });
});
